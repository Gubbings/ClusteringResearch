...
class LocalSearchClustering2 {
public:
	LocalSearchClustering2(string graphFile, string clusterFile, string outputFile);
	~LocalSearchClustering2();
	void beginClustering();

private:
	string *vertexNames;
	uint **graph, *adjVertsCount, *clusterSize;
	float **vertexWeights;
